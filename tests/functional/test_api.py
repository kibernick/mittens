...
class TestLogList:
    @pytest.mark.parametrize(
        ...
        [
            ({"id": 1, "content": "bla"},),
            ({"id": 1, "content": "bla"}, {"id": 2, "content": "bla", "meta": {"7": 9}}),
        ],
    )
    def test_get_all_logs(self, app, session, logs, client, tenant):
        ...
        res = client.get(url_for("log-list"), headers={"Authorization": f"Basic {tenant.api_key}"})
    ...
    def test_do_not_retrieve_other_logs(self, app, session, client, tenant):
        ...
        res = client.get(url_for("log-list"), headers={"Authorization": f"Basic {tenant.api_key}", "X-Fields": "id"})
        assert res.status_code == 200
        assert res.json == [{"id": my_log.id}]

    def test_list_with_wrong_tenant(self, app, session, client, tenant):
        res = client.get(url_for("log-list"), headers={"Authorization": f"Basic Z{tenant.api_key}"})
...
class TestLogDetails:
    ...
    def test_get_log_details(self, app, session, existing, client, tenant):
        ...
        res = client.get(url_for("log-details", log_id=_log.id), headers={"Authorization": f"Basic {tenant.api_key}"})
        assert res.status_code == 200
    ...
    def test_get_nonexistent(self, app, session, client, tenant):
        ...
        res = client.get(url_for("log-details", log_id=1), headers={"Authorization": f"Basic {tenant.api_key}"})
        assert res.status_code == 404

    def test_get_with_wrong_tenant(self, app, session, client, tenant):
        res = client.get(url_for("log-details", log_id=1), headers={"Authorization": f"Basic Z{tenant.api_key}"})
        assert res.status_code == 401
